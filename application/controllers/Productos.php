...
class Productos extends CI_Controller
{
    ...
    public function productos($status_producto = 1)
    {
        ...
        $sql = "SELECT  
                    ...
                    p.precio_compra,
                    p.tarifa_venta, 
                    p.margen_real_producto, 
                    p.stock_total, 
                    p.valoracion, 
                    p.stock_total, 
                    p.url_imagen_portada, 
                    pr.nombre_proveedor as proveedor FROM pe_productos p
                    LEFT JOIN pe_proveedores pr ON p.id_proveedor_web=pr.id_proveedor
                    WHERE status_producto='$status_producto'
                    ORDER BY p.codigo_producto ";
        ...
        // $dato['tabla'] = $this->prepararTabla($this->db->query($sql)->result(),$status_producto);
    ...
    function prepararTabla($productos, $status_producto)
    {
        $tabla = "";
        $tabla .= '<table id="productos" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" style="display:none">';
        $tabla .= '<thead>';
        $tabla .= '<tr>';
        $tabla .= '<th></th>';
        $tabla .= '<th>Código Producto</th>';
        $tabla .= '<th>Cód. Bascula</th>';
        $tabla .= '<th class="izquierda" style="min-width:40% !important">Nombre</th>';
        $tabla .= '<th>Peso Real (Kg)</th>';
        $tabla .= '<th>Tipo Unidad</th>';
        $tabla .= '<th>Precio Compra Final en Tienda</th>';
        $tabla .= '<th class="izquierda" style="min-width:20% !important">Proveedor</th>';
        $tabla .= '<th>Tarifa PVP</th>';
        $tabla .= '<th>Margen (%)</th>';
        $tabla .= '<th>Undidades Stock</th>';
        $tabla .= '<th>Valor Stock precio compra actual</th>';
        $tabla .= '<th>Imagen Producto</th>';
        $tabla .= '</tr>';
        $tabla .= '</thead>';
        $tabla .= '<tbody>';

        foreach ($productos as $k => $v) {
            $tabla .= '<tr producto=' . $v->id . '>';
            $tabla .= '    <td >';
            $tabla .= '        <!--Dropdown -->';
            $tabla .= '        <div class="dropdown">';
            $tabla .= '            <!--Trigger-->';
            $tabla .= '            <a class="btn btn-sm btn-blue-grey dropdown-toggle acciones" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Acciones </a>';
            $tabla .= '            <!--Menu-->';
            $tabla .= '            <div class="dropdown-menu dropdown-light">';
            $tabla .= '                <a class="dropdown-item btn-sm editar" href="#">Editar</a>';
            $tabla .= '                <a class="dropdown-item ver" href="#">Ver producto</a>';
            $desCat = $status_producto ? " Descatalogar" : " Catalogar";
            $tabla .= '                <a class="dropdown-item ' . strtolower($desCat) . '" href="#">' . $desCat . '</a>';
            $tabla .= '                <a class="dropdown-item eliminar" href="#">Eliminar</a>';
            $tabla .= '            </div>';
            $tabla .= '        </div>';
            $tabla .= '    </td>';
            $tabla .= '    <td class="text-center">' . $v->codigo_producto . '</td>';
            $tabla .= '    <td class="text-right">' . $v->id_producto . '</td>';
            $tabla .= '    <td class="text-left">' . $v->nombre . '</td>';
            $tabla .= '    <td class="text-right"' . number_format($v->peso_real / 1000, 3, ",", ".") . '</td>';
            $tabla .= '    <td class="text-right"' . $v->tipo_unidad . ' </td>';
            $tabla .= '    <td class="text-right"' . number_format($v->precio_compra / 1000, 3, ",", ".") . '</td>';
            $tabla .= '    <td class="text-left">' . $v->proveedor . '</td>';
            $tabla .= '    <td class="text-right">' . number_format($v->tarifa_venta / 1000, 3, ",", ".") . '</td>';
            $tabla .= '    <td class="text-right">' . number_format($v->margen_real_producto / 1000, 3, ",", ".") . '</td>';
            $tabla .= '    <td class="text-right">' . $v->stock_total . '</td>';
            $tabla .= '    <td class="text-right">' . number_format($v->valoracion, 3, ",", ".") . '</td>';
            // $tabla.='    <td><img class=" img rounded-circle" src="'.$v->url_imagen_portada .'"  /></td>';
            $tabla .= '    <td></td>';
            $tabla .= '    <td></td>';
        ...
        $tabla .= '            <th>Código Producto</th>';
        $tabla .= '            <th>Cód. Bascula</th>';
        $tabla .= '            <th class="izquierda">Nombre</th>';
        $tabla .= '            <th>Peso Real (Kg)</th>';
        $tabla .= '            <th>Tipo Unidad</th>';
        $tabla .= '            <th>Precio Compra Final en Tienda</th>';
        $tabla .= '            <th class="izquierda">Proveedor</th>';
        $tabla .= '            <th>Tarifa PVP</th>';
        $tabla .= '            <th>Margen (%)</th>';
        $tabla .= '            <th>Undidades Stock</th>';
        $tabla .= '            <th>Valor Stock precio compra actual</th>';
    ...
    function getEditarProductoModal()
    {
        ...
        mensaje('categoria ' . $this->session->categoria);

        if ($this->session->categoria == 2) {
            $grid[0] = array(0, 1, 2, 5, 17);
            $grid[1] = array(3, 4);
            $grid[2] = array(6, 7, 8, 9);
            $grid[3] = array(10, 11, 12, 13);
            $grid[4] = array(24, 25, 30);
            $grid[5] = array(31, 32);
        } else {
            $grid[0] = array(0, 1, 2, 5, 17);
            $grid[1] = array(3, 4);
            ...
            foreach ($fila as $columna) {
                switch ($estructura[$columna]['tipo']) {
                    ...
                    case 'number':
                        ...
                        mensaje('<input type="text" id="' . $estructura[$columna]['campo'] . '" class="form-control" ' . $estructura[$columna]['editar'] . ' value="">');
                        break;
                    case 'seleccion':
                        $sm = "";
